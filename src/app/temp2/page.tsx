...
const ReportView = () => {
        ...
        <div className="flex justify-between items-center mb-4 print:hidden bg-white p-4 rounded-lg shadow-sm print-hide">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={printing}
              className="px-3 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors flex items-center gap-2 disabled:opacity-50"
            >
              {printing ? (
  ...
  <img src="/curehospital.png" alt="Watermark" className="w-2/3 max-w-xs md:max-w-md lg:max-w-lg object-contain" />
</div>



              {/* Header */}
              <div className="flex justify-between items-start border-b border-green-100 pb-6 mb-6 relative z-10">
                <div className="flex items-center">
                  <img src="/curehospital.png" alt="Lab Logo" className="h-14 mr-4 print:h-12" />
                  <div>
                    <h1 className="text-2xl font-bold text-green-800 print:text-xl">{currentLab?.name || ''}</h1>
                    <p className="text-xs text-gray-600 mt-1 print:text-[10px]">Accredited by NABL | ISO 15189:2012 Certified</p>
                  </div>
                </div>
                <div className="text-right bg-green-50 p-3 rounded-lg print:p-2">
                  <p className="text-xs font-medium text-green-700 print:text-[10px]">Report ID: <span className="font-bold">{viewPatient?.visitId || 'N/A'}</span></p>
                  <p className="text-xs font-medium text-green-700 print:text-[10px]">Date: <span className="font-bold">{new Date().toLocaleDateString()}</span></p>
                  <p className="text-xs font-medium text-green-700 mt-1 print:text-[10px]">Page: {index + 1}/{Object.keys(groupedReports).length}</p>
                </div>
              </div>

              {/* Patient Info */}
              <div className="grid grid-cols-4 gap-4 mb-6 bg-green-50 p-4 rounded-lg border border-green-100 text-sm print:p-2 print:text-xs relative z-10">
                ...
                {/* Demographics */}
                <div className="space-y-1">
                  <p className="font-medium text-green-700">Age /Gender</p>
                  <div className="flex gap-2">
                ...
                {/* Physician Info */}
                <div className="space-y-1">
                  <p className="font-medium text-green-700">Referred By</p>
              ...
              <div className="mb-6 relative z-10">
                <h2 className="text-xl font-bold text-green-800 mb-2 print:text-lg">{testName}</h2>
                <div className="h-1 bg-gradient-to-r from-green-400 to-green-100 rounded-full"></div>
              </div>
              ...
              <div className="mt-auto pt-6 border-gray-200 print:pt-4 relative z-10">
                ...
                <div className="mt-4 text-center print:mt-2">
                  ...
                  <p className="text-xs font-medium text-green-600 mt-2 print:text-[10px]">Thank you for choosing NEXTJEN DIAGNOSTICS</p>
                </div>
              </div>
